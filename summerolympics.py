...
import pandas as pd
import preprocessorsummer, helper
...
def run():
    ...
    df = preprocessorsummer.preprocess(df1, df2)

    st.sidebar.title('Summer Olympics Analysis(1896-2020)')
    st.sidebar.image('https://assets.weforum.org/article/image/NEAE0GKH4tQ9RGjrC4JSjXbaKgg8Tn4lgYW-N6Fk9Pc.jpg')
    ...
    if user_menu == 'Overall Analysis':
        st.title("Olympics Statistics (1896–2020)")
    ...
    if user_menu == 'Country wise Analysis':
        ...
        if pt is None or pt.empty:
            ...
            fig, ax = plt.subplots(figsize=(20, 20))
            sns.heatmap(pt, annot=True, fmt="g")  # fmt="g" ensures integers display properly
            st.pyplot(fig)
